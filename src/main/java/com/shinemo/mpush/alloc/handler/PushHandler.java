...
public class PushHandler implements HttpHandler {
  ...
  private void sendPush(Map<String, Object> params) {
    ...
    PushContext pushContext = PushContext.build(pushMsg).setUserId(Strings.isBlank(userId) ? null : userId)
      .setBroadcast(broadcast != null && broadcast).setCondition(Strings.isBlank(condition) ? null : condition)
